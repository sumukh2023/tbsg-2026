...
for (const [who, email, role, home] of [
  ['administrator', 'sumukh.nayak@outlook.com', 'admin', '/verify-pass/admin'],
  ['volunteer', 'ryansahatbsg@gmail.com', 'volunteer', '/verify-pass'],
]) {
  ...
  await page.goto(`http://localhost:${APP_PORT}/verify-pass`, { waitUntil: 'networkidle' });
  await page.waitForTimeout(700);
  check('redirected to the login page',
    new URL(page.url()).pathname === '/verify-pass/login', page.url());
...
console.log('\ncookies blocked — the failure must be legible');
{
  ...
  const ctx = await browser.newContext({ viewport: { width: 900, height: 1000 } });
  const page = await ctx.newPage();
  await page.goto(`http://localhost:${APP_PORT}/verify-pass/login`, { waitUntil: 'networkidle' });
