...
- `src/App.tsx` — routes + HomePage section order (Hero → Overture → PiazzaBento
  → Regions → Programme → Italia → Voci → Giorno → Domande →
...
- `src/festival/italia/` — **L'Italia**, the interactive map that replaced Il
  Mercato on the landing page (5 Aug 2026). The mercato itself was not
  deleted; it lives on `/stalls`.
  - `geometry.ts` is GENERATED by `node scripts/make-italy-map.mjs` from
    Natural Earth 1:10m via world-atlas — do not hand-edit. It exports the
    real coastline AND `project(lat, lon)`, the same Web Mercator transform
    the outline came out of, so a marker given real coordinates lands where
    it belongs. Neither generator dependency is installed by `npm install`;
    the output is committed.
  - `places.ts` — sixteen places. **Landmarks are deliberately not markers**:
    Rome's eight and the Vatican occupy the same pixel and a half at this
    scale, so the city is the marker and its landmarks are what the card
    reveals. `side` and `nudge` are hand-tuned against measured label boxes.
  - The card's per-place backdrop is a colour plate, not a photograph — there
    is no licensed image of the Colosseum in this repo and no generator
    reachable from a cloud session. Giving a place a `photo` field is the
    whole seam for the real thing.
