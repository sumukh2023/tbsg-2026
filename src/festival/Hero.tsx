    ...
    >
      {arches.map((arch, i) => (
        <motion.path
          key={arch.d}
          d={arch.d}
          className={i === 0 ? 'stroke-accent/50' : 'stroke-foreground/15'}
...
export function Hero() {
  ...
  // The hero hands over to the next chapter: copy lifts and dissolves while
  // the colonnade drifts slower, giving the scroll a sense of depth.
  const contentY = useTransform(scrollYProgress, [0, 1], [0, -120]);
  const contentOpacity = useTransform(scrollYProgress, [0, 0.6], [1, 0]);
  const archDrift = useTransform(scrollYProgress, [0, 1], [0, 60]);
  ...
  return (
    ...
    >
      {/* Warm midday light falling into the piazza */}
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[radial-gradient(90%_60%_at_50%_-5%,hsl(var(--accent)/0.16),transparent_70%)]"
      />
      <div
        aria-hidden="true"
        className="absolute inset-0 bg-[radial-gradient(70%_45%_at_50%_110%,hsl(var(--primary)/0.10),transparent_70%)]"
      />
      <MarbleVeins className="opacity-[0.10]" />
      <Grain />

      <motion.div style={{ y: archDrift }} className="absolute inset-0">
        <Colonnade />
      </motion.div>
