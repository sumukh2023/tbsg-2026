  ...
  return (
    <section id="attractions" className="mx-auto max-w-6xl px-6 py-24">
      ...
      <div ref={ref} className="mt-12">
        {inView && (
          <AnimatedGroup preset="blur-slide" className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {BLOCKS.map((b) => (
              <article
                key={b.title}
                className={cn(
                  'relative flex flex-col overflow-hidden rounded-2xl border-2 border-foreground p-7 shadow-[5px_5px_0_0_hsl(var(--foreground))]',
                  TONE[b.tone],
                  b.wide && 'sm:col-span-2 lg:row-span-2 justify-between'
                )}
              >
